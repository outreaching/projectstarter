...
contract ProjectStarterLaunchPad is Ownable, constructorLibrary {
    ...
    function withdrawBNBEmergency(address payable recipient, uint256 amount)
        public 
        onlyOwner
    {
    ...
    function isEligible(address toCheckAddress) public view returns (uint256) {
        ...
        if (
            tokens >= amountRequiredTier1 ||
            whitelistTierOne[toCheckAddress]
        ) {
            return 1;
        } else if (
            tokens >= amountRequiredTier2 ||
            whitelistTierTwo[toCheckAddress]
        ) {
            return 2;
        } else if (
            tokens >= amountRequiredTier3 ||
            whitelistTierThree[toCheckAddress]
        ) {
            return 3;
        } else {
            return 0;
    ...
    function isEligibleInTier1(address toCheckAddress)
        public
        view
        returns (bool)
    {
    ...
    function isEligibleInTier2(address toCheckAddress)
        public
        view
        returns (bool)
    {
    ...
    function isEligibleInTier3(address toCheckAddress)
        public
        view
        returns (bool)
    {
